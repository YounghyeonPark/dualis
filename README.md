...
| `dualis` | A facade over the other fourteen, and where the cross-domain integration tests live |
...
| `dualis-world` | The first consumer, and not published. Worlds described as data: built, coupled over the bus, run and drawn, with eighteen scenes across seven of the ten domains that CI runs. It exists to use the SDK from outside and write down where that is awkward |
