...
def test_units_crossed_intact():
    assert pantometry.one_joule() == 1.0
    ...
    assert pantometry.__version__ == version("pantometry"), (
        f"Cargo.toml says {pantometry.__version__}, pyproject.toml says {version('pantometry')}"
    )
...
def test_every_joule_arrives_and_the_rise_matches_the_closed_form():
    ...
    capacity = pantometry.heat_capacity_j_per_k("aluminium", 0.020 * 1e-4)
    want = 6.0 / capacity
...
def test_the_mistakes_a_caller_will_make_are_named():
    ...
    for call, fragment in [
        (lambda: pantometry.Simulation(schedule="magic"), "unknown schedule"),
...
def motor(watts=6.0, seconds=900):
    ...
    sim = pantometry.Simulation(schedule="staggered", conservation_tolerance=1e-9)
...
def test_a_network_of_one_node_matches_the_lump_it_reduces_to():
    ...
    a = pantometry.Simulation(schedule="staggered", conservation_tolerance=1e-9)
    a.add_lump("plate", volume_m3=volume, thickness_m=thickness, area_m2=area,
               initial_k=358.15, ambient_k=293.15)

    b = pantometry.Simulation(schedule="staggered", conservation_tolerance=1e-9)
    b.add_network(
        "plate",
...
def test_a_bar_and_a_lump_can_be_made_of_something_other_than_aluminium():
    ...
    for material in ["aluminium", "copper", "stainless_304"]:
        ...
        while sim.reserve_j("src") > 0.0:
            sim.advance(0.5)
        want = 293.15 + joules / pantometry.heat_capacity_j_per_k(material, volume)
...
def test_every_catalogue_material_can_be_named():
    ...
    lookup is not a wrong answer, it is a substance that never appears. `pantometry-world` had the identical
    ...
    for material in known:
        sim = pantometry.Simulation(schedule="one-way", conservation_tolerance=1e-9)
        sim.add_network(
            f"net_{material}",
