...
//!
//! `pantometry-optics` computes a surface's reflectance from Fresnel's equations — algebra on two
//! refractive indices. `pantometry-em` marches Maxwell on a grid and knows nothing about either. The
