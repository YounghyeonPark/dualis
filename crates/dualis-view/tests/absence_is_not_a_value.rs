...
//! decides what is absent has its own, in `dualis-thermal`.
...
fn the_gltf_leaves_the_holes_out() {
    let doc = dualis_view::gltf("holed", &holed()).document;
    assert!(
...
fn the_json_writers_spell_absence_null_and_leave_it_where_it_was() {
    for (what, text) in [
        ("to_json", dualis_view::to_json("holed", &[holed()])),
        ("html", dualis_view::html("holed", &[holed()])),
    ] {
...
fn the_svg_draws_no_square_for_an_empty_cell() {
    let svg = dualis_view::svg("holed", &[holed()], 1);
    assert_eq!(
