...
fn root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("crates/pantometry-world has two ancestors")
        .to_path_buf()
}
...
fn friction_totals() -> Option<(usize, usize)> {
    let text = std::fs::read_to_string(root().join("crates/pantometry-world/FRICTION.md")).ok()?;
    let findings = text
        .lines()
        .filter(|l| {
            l.strip_prefix("## ")
...
fn the_findings_total_agrees_everywhere_it_is_written() {
    ...
    phrase(
        "crates/pantometry-world/src/lib.rs",
...
fn the_scene_count_agrees_everywhere_it_is_written() {
    let dir = root().join("crates/pantometry-world/scenes");
    ...
    let readme = "crates/pantometry-world/scenes/README.md";
