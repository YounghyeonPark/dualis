...
//!
//! `dualis-optics` publishes absorbed light as heat and `dualis-thermal` consumes it.
//! That could have been an arrangement between two crates written together. So here
//! the publisher is `dualis-mechanics` instead: a bouncing ball's dashpot dissipates
...
fn a_bouncing_ball_warms_itself() {
    ...
    let mut sim = Simulation::new(Schedule::Multirate)
        ...
        // dualis-mechanics on why this number is the integrator's and not the audit's.
...
fn the_kernel_polices_momentum_as_readily_as_energy() {
    ...
    for axis in [
        dualis::mechanics::conserved::MOMENTUM_X,
        dualis::mechanics::conserved::MOMENTUM_Y,
        dualis::mechanics::conserved::MOMENTUM_Z,
    ] {
