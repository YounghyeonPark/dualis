...
/// `pantometry-thermal` takes from — so a winding published joules that nothing consumed, in a
...
impl Winding {
    ...
    /// [`ThermalNetwork`](https://docs.rs/pantometry-thermal) over one body.
    ///
...
impl Domain for Winding {
    ...
    fn step(&mut self, _t: Time, dt: Time, bus: &mut Exchange) -> Result<(), Violation> {
        ...
        // repository has shipped that failure once already, in `pantometry-world`'s lamp, where a
