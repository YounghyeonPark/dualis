...
//!
//! The speeds themselves are algebra and are checked in `crates/dualis/tests/two_wave_speeds.rs`.
...
fn the_marched_ratio_is_poissons_ratio_and_the_mesh_error_cancels() {
    for nu in [0.2, 0.33, 0.45] {
        let material = Elastic::new(
            dualis_core::units::Pressure::from_si(68.9e9),
            nu,
            dualis_core::units::Density::from_si(2700.0),
        )
