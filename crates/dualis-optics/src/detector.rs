...
use dualis_core::Rng;
use dualis_units::{Energy, Frequency, Length, Power, Time};
...
impl Detector {
    ...
    pub fn energy_per_electron(&self, wavelength: Length) -> Energy {
        ...
        Energy::from_si(dualis_units::photon_energy(wavelength).to_si() / qe)
    }
    ...
    pub fn noise_equivalent_power(
        ...
        let energy = dualis_units::photon_energy(wavelength).to_si();
...
mod tests {
    ...
    fn a_count_becomes_watts_through_the_efficiency_and_the_photon_energy() {
        ...
        let photon = dualis_units::photon_energy(nm(550.0));
