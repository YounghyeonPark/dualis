...
//! this is not a [`Newtonian`](pantometry_core::Newtonian) system and there is no
//! symplectic method for it. It goes through
//! [`Integrator::Rk4`](pantometry_core::Integrator), which is fourth-order accurate and
...
use pantometry_core::{Domain, Dynamics, Exchange, Integrator, Kind, Ledger, State, Violation};
use pantometry_units::{AngularMomentum, Energy, Length, Mass, MomentOfInertia, Time};
...
pub struct RigidBody {
    ...
    ///
    /// A `DVec3` rather than a dimensioned type, for the reason `pantometry-units`
