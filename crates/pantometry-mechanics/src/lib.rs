//! pantometry-mechanics: motion under force, as a domain on the `pantometry-core` kernel.
//!
...
//!   second customer for [`audit`](pantometry_core::audit) after energy, and the test
...
//!   channel `pantometry-thermal` already consumes from `pantometry-optics`. Neither
...
//! and exposes energy as a method instead: [`audit`](pantometry_core::audit) applies one
...
///
/// Deliberately the same name `pantometry-thermal` consumes: friction produces heat, and
...
impl Body {
    ...
    pub fn kinetic_energy(&self) -> Energy {
        pantometry_units::vector::kinetic_energy(self.mass, self.velocity)
    }
...
impl NBody {
    ...
    pub fn momentum(&self) -> pantometry_units::MomentumVec {
        let p = self
            ...
            .map(|(m, v)| *v * *m)
            .fold(DVec3::ZERO, |a, b| a + b);
        pantometry_units::MomentumVec::from_si(p)
    }
...
impl Domain for NBody {
    ...
    /// largest single contribution as an entry's `scale`, and [`audit`](pantometry_core::audit)
...
///   [`Schedule::Multirate`](pantometry_core::Schedule::Multirate) is for.
...
impl ContactSystem {
    ...
    pub fn new(
        ...
        bodies: &[Body],
        gravity: pantometry_units::AccelerationVec,
        ground: Ground,
        stiffness: Stiffness,
        damping: Damping,
    ) -> ContactSystem {
...
impl Domain for ContactSystem {
    ...
    /// mirror image of the mistake `pantometry-thermal` made in the other direction — a
    ...
    ///
    /// [`Exchange::audit_transfers`](pantometry_core::Exchange::audit_transfers) is what
    ...
    /// `crates/pantometry/tests/friction_heats.rs`, which made exactly that compromise.
    fn as_bodies(&self) -> Option<&dyn Bodies> {
        Some(self)
    }
...
pub fn momentum_magnitude(p: pantometry_units::MomentumVec) -> Momentum {
    p.length()
}
...
mod tests {
    use super::*;
    use pantometry_core::{audit, Schedule, Simulation};
    use pantometry_units::{AccelerationVec, G0};
