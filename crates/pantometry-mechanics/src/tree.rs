...
use glam::DVec3;
use pantometry_core::{
    velocity_verlet, Bodies, Domain, Exchange, Kind, Ledger, Newtonian, Violation,
};
use pantometry_units::{Energy, Length, LengthVec, Mass, Time};
...
impl TreeNBody {
    ...
    pub fn body(&self, index: usize) -> Body {
        Body {
            ...
            position: pantometry_units::LengthVec::from_si(self.positions.0[index]),
            velocity: pantometry_units::VelocityVec::from_si(self.velocities.0[index]),
        }
    }
...
mod tests {
    ...
    use pantometry_units::{LengthVec, VelocityVec};
