...
use pantometry_units::{Length, Pressure};
...
fn the_interior_reproduces_a_linear_field_prescribed_on_the_boundary() {
    ...
    let field = |p: pantometry_units::LengthVec| {
        let v = p.to_si();
        pantometry_units::LengthVec::from_si(glam::DVec3::new(
    ...
    for k in 1..nz - 1 {
        for j in 1..ny - 1 {
            for i in 1..nx - 1 {
                let at = pantometry_units::LengthVec::from_si(
...
fn simple_shear_costs_half_mu_gamma_squared() {
    ...
    b.prescribe_boundary(|p| {
        pantometry_units::LengthVec::from_si(glam::DVec3::new(gamma * p.to_si().y, 0.0, 0.0))
    });
    assert!(b.solve(1e-14), "residual {:.3e}", b.residual());
...
fn an_unsolvable_body_refuses_its_step() {
    ...
    let err = b
        .step(
            pantometry_units::Time::from_si(0.0),
            pantometry_units::Time::from_si(1.0),
            &mut pantometry_core::Exchange::new(),
        )
...
fn a_solved_body_offers_its_displacement_as_a_field() {
    use pantometry_core::units::{LengthVec, Time};
...
fn a_substance_converts_to_a_material_and_the_yield_strain_is_small() {
    ...
    assert!(
        Elastic::from_substance(&Substance::bulk(
            "mystery",
            pantometry_core::units::Density::g_per_cm3(2.0)
        ))
        .is_none(),
