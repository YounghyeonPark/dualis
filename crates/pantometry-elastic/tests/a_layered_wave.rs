...
//! 32 shows the same rate. `pantometry-porous` records the same lesson about a jacket at 1 mm — a test that
//! costs more than the eleven beside it is a test somebody will stop running.

use pantometry_core::mixture::Mix;
use pantometry_core::substance::Substance;
use pantometry_core::units::{Length, Time, Velocity};
...
fn a_laminate_gives_both_shear_bounds_from_one_block() {
    ...
    // `pantometry-porous` records the same lesson: a test that costs more than the eleven beside it is a
