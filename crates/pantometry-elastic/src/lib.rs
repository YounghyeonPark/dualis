...
//!
//! `pantometry-mechanics` moves bodies that do not change shape. This is the other half — a body that
...
//! ```
//!
//! Elliptic, like [`Conductor`](https://docs.rs/pantometry-electrical) and
//! [`Puck`](https://docs.rs/pantometry-porous), and solved the same way — conjugate gradients on a
...
//! element, marched with central differences. `Block` stays [`Kind::QuasiStatic`](pantometry_core::Kind)
...
use pantometry_units::{Density, Pressure, Velocity};
...
impl Elastic {
    ...
    ///
    /// `None` for a fluid: [`Substance::water`](pantometry_core::Substance::water) has no `mechanical`
    ...
    /// [`AcousticProps`](pantometry_core::substance::AcousticProps) has room for one speed and a solid
    /// has two.
