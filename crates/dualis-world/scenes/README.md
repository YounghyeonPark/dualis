...
Twenty-seven worlds described as data, covering ten of the library's eleven domains — twenty-five of them
...
Every file carries a `format` number, and **absence means 1** — which is what all twenty-seven here
...
`.csv` is the one that reaches the domains a picture cannot. Twelve of these twenty-seven scenes have
...
`.gltf` is the one that leaves this workspace. Eighteen of the twenty-seven scenes have geometry to
...
than none at all. `tests/scene.rs` runs all twenty-seven on every commit and asserts one number each —
